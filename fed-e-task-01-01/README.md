...
a[6]()
```
*答：* i执行完后为9，此时i任小于10，变量提升，执行i++，得到i=10,所以无论a[*]()中，索引为任意值，i输出结果都为10。
...
```
*答：* ReferenceError: Cannot access 'tmp' before initialization,在ES6中如果使用let或const声明变量，那么一对大括号界定的区域就是块级作用域，并且变量不存在提升的可能性
...
```
*答：* 利用内置对象方法：Math.min和展开运算符，代码如下：
```js
var arr = [12, 34, 32, 89, 4]
...
obj.fn()
```
*答：* 20，箭头函数表达式没有自己的this，它只会从自己的作用域链的上一层继承this，即20
